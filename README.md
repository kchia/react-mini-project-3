...
To run the app from your terminal, cd into the root directory, install all dependencies with `npm install`and then run the command `npm start` (assuming you already have npm and node installed on your computer) from your root directory. You should see the app running at localhost:3000. 

If you don't have Node on your computer, then please install Node first, following the instructions here: https://www.npmjs.com/get-npm. 

This project was bootstrapped with https://github.com/facebookincubator/create-react-app. The following files contain the code I authored for the coding challenge:

- src/Components/App.js
- src/Components/AssetCorrelationsTable.js
- src/Components/SelectTickers.js
- src/App.css

For this project, I focused on good code standards for maintainability and readability. I added proper documentation to the project while writing modular code using React components. I also leveraged ES6 for this project and attempted to handle as many edge cases as possible (see comments in the code) . However, I did not focus much on visuals/CSS/aesthetics so please pardon the app's lack of visual appeal :| 

This app provides a simple interface for financial professionals to explore correlations between stock tickers. Here's an explanation of how it works:

- When you initially navigate to localhost:3000, you'll be presented with an empty state. "You've selected these tickets to compare:" is set to "None" since no ticker has been selected. Assets correlation table is also not displayed in the empty state.
- To select tickers for comparison, click on the dropdown menu "--select an option" and select an option. Selecting only 1 ticker will cause an error message to appear. Selecting the next ticker will cause two things to happen: 1. The error message will go away, and 2. the assets correlation table is rendered. Selecting more tickers will cause the assets correlation table to expand dynamically, with the most positively correlated and negatively correlated values being updated as well
- Clicking on the "Regenerate Asset Correlations" button will refresh the table with the most up-to-date correlation values from the server
- Clicking on the "Restart" resets the app and brings the user back to the empty state
